...
class DropoutNormal(RandomVariable, Distribution): 
    ...
    def _log_prob_no_logsum(self, value):

        is_zero = tf.equal(value, tf.zeros(value.shape))
 
        # If value == 0
        log_p_dropout = tf.log(self._p_dropout)
        log_p_truezero = tf.log(1 - self._p_dropout) + Normal(loc = self._loc, scale = self._scale)._log_prob(value)
        log_p_zero = tf.reduce_mean(tf.stack([log_p_dropout, log_p_truezero]), axis = 0) 

        # If value > 0
        log_p_nonzero = (tf.log(1 - self._p_dropout) + Normal(loc = self._loc, scale = self._scale)._log_prob(value))

        reduction_tensor = tf.where(is_zero, log_p_zero, log_p_nonzero)
        # return tf.reduce_sum(reduction_tensor)
        return reduction_tensor

    def _log_prob_no_where(self, value):
        # lp_amp = tf.log1p(-self._p_dropout)
        lp_amp = -self._p_dropout - tf.log1p(tf.exp(-self._p_dropout))
        lp = tf.add(lp_amp, Normal(loc = self._loc, scale = self._scale)._log_prob(value))

        return lp

    def _log_prob_no_dropout(self, value):

        return Normal(loc = self._loc, scale = self._scale)._log_prob(value)
