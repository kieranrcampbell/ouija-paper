...
generate_dataset <- function(C, G) {
  ...
  t0 <- runif(G, 0.1, 0.9)
  mu0 <- runif(G, 3, 4)
  k <- runif(G, 10, 50) * sample(c(-1, 1), G, replace = TRUE)
  
  Y <- mapply(rsinglecell, k, mu0, t0, MoreArgs = list(pst = true_pst))
  
  return( list(d = data.frame(Y, pseudotime = true_pst), k_sgn = sign(k), t0 = t0, mu0 = mu0) )
}
...
h5_file <- "data/benchmarking/probit_synthetic.h5"
...
ggsave(gene_plt, file = "figs/example_genes_probit.png", width=8, height=4)
